...
fluidPage(sidebarLayout(sidebarPanel(
  ...
  numericInput("html", "HTML/CSS developers", 17),
  numericInput("json", "JSON users", 17),
  numericInput("rstats", "R Statistical langauge", 17)  
  ),
  mainPanel(
    renderPlot({
      techavg <- mean(c(input$html,input$json,input$rstats) / input$members)*100
  df1 <- data.frame(units = c("HTML(%)","JSON(%)","R(%)"),
                    low = c(25,25,25),
                    mean = c(50,50,50),
                    high = c(100,100,100),
...
shiny web application using bullet charts for R by https://github.com/sipemu/BulletGraph
